...
from models.mca import MultiCueAttention
...
class DehazeFormer(nn.Module):
	def __init__(self, in_chans=3, out_chans=4, window_size=8,
		...
		self.mca = MultiCueAttention(embed_dims[0])
	...
	def forward(self, x):
		...
		illum = torch.mean(x, dim=1, keepdim=True)

		# haze level
		haze_level = 1 - torch.std(x, dim=[2,3], keepdim=True)
		...
		illum_norm = (illum - illum.min()) / (illum.max() - illum.min() + 1e-6)

		# depth-aware brightness ONLY
		weight = 1 - illum_norm
		boost = (0.08 + 0.25 * haze_level) * weight
		x = x + boost * illum

		# === contrast (mild, safe) ===
		mean = torch.mean(x, dim=[2,3], keepdim=True)
		x = (x - mean) * (1.1 + 0.3 * haze_level) + mean

		# === STRONG COLOR RESTORATION (independent of depth) ===
		gray = torch.mean(x, dim=1, keepdim=True)
		
		# softer saturation (reduced from 0.4 → 0.25)
		x = x + 0.40 * (x - gray)

		# === color balance (fix yellow/green tint) ===
		r_mean = torch.mean(x[:,0:1,:,:])
		g_mean = torch.mean(x[:,1:2,:,:])
		b_mean = torch.mean(x[:,2:3,:,:])

		mean = (r_mean + g_mean + b_mean) / 3

		x[:,0:1,:,:] *= mean / (r_mean + 1e-6)
		x[:,1:2,:,:] *= mean / (g_mean + 1e-6)
		x[:,2:3,:,:] *= mean / (b_mean + 1e-6)

		# === slight channel balance (fix color cast) ===
		mean_rgb = torch.mean(x, dim=[2,3], keepdim=True)
		x = x / (mean_rgb + 1e-6) * torch.mean(mean_rgb)
